...
export default class RectOminoEl extends OminoEl<RectOrienVal>{
    ...
    private readonly flipH = button(_=>{
        ...
        while(childrenToParse.length>0){
            const left = childrenToParse[0]!.left;
            const right = childrenToParse[0]!.right;
            if(left) {
                childrenToParse[0]!.right = left;
                delete childrenToParse[0]!.left;
            }
            if(right) {
                childrenToParse[0]!.left = right;
                if(!left) delete childrenToParse[0]!.right;
            }
    ...
    private readonly flipV = button(_=>{
        ...
        const childrenToParse:ConnTree<any, RectOrienVal>[] = [this.connTree];
        while(childrenToParse.length>0){
            const up = childrenToParse[0]!.up;
            const down = childrenToParse[0]!.down;
            if(up) {
                childrenToParse[0]!.down = up;
                delete childrenToParse[0]!.up;
            }
            if(down) {
                childrenToParse[0]!.up = down;
                if(!up) delete childrenToParse[0]!.down;
            }
            for(const child in childrenToParse[0]){
                childrenToParse.push(childrenToParse[0][child as RectOrienVal]!);
            }
    ...
    constructor(connTree:ConnTree<Orientation<RectOrienVal>, any>, root:number, orientation:Orientation<RectOrienVal>) {
