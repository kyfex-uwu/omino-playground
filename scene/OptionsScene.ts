...
import ChangeBoardScene from "omino/scene/ChangeBoardScene.js";
...
class OptionsScene extends DimsScene<any> {
    ...
    constructor(board:Board) {
        ...
        this.bottomBar = this.addScene(new Bar(
            new OneTimeButtonScene((s, env) => {
                ...
                let offs = s.isIn() ? 1.1 : 1;
                fill("scenes.buttons.dark.icon", env);
                env.scale(s.dims.x/100, s.dims.y/100);
                env.beginPath();
                env.roundRect(5,5,90,90, 16);
                env.clip();
                env.scale(offs, offs);
                env.translate((1-offs)*50,(1-offs)*50);
                env.beginPath();
                for(let y=0;y<3;y++){
                    for(let x=0;x<3;x++){
                        env.roundRect(x*20,y*20-4,16,16, 3);
                        env.ellipse(x*18+56,y*18-x*9-4+75,8,8, 0,0,Math.PI*2);
                    }
                }
                env.fill();

                if (s.isIn()) hover.set("Change Board", s);
            }, () => {
                if(data.scene instanceof MainScene)
                    data.scene = new ChangeBoardScene(data.scene);
            }),//change board
            new OneTimeButtonScene((s, env) => {
                barButtonWrapper(s, env);
    ...
    resized(old:Vector, n = old) {
        ...
        const unit = this.dims.x/this.bottomBar.subScenes.length;
        for(let i=0;i<this.bottomBar.subScenes.length;i++){
            this.bottomBar.subScenes[i]!.dims.replace(sbSize * 0.8, sbSize * 0.8);
            this.bottomBar.subScenes[i]!.pos.replace(this.dims.x/2+(i-this.bottomBar.subScenes.length/2+0.5)*unit-sbSize*0.4, sbSize * 0.1);
        }
