...
const changelog = [
    `v1.0.3 3/13/26
- Fixed the edit dialog acting strange + added it to hex ominoes`,
    `v1.0.2 3/13/26
- Added a "view source" button, for anyone who wants to tinker with the code :3`,
    `v1.0.1b 3/13/26
...
- ported omino playground to TypeScript
- added portals!
...
function smartText(text:string, env:AnyEnhancedEnv, x:number, y:number, w:number) {
    ...
    for (const line of splitText) {
        ...
        for (const section of line) {
            ...
            if (env.measureText(newLine.join("") + toAdd).width < w) {
                newLine.push(toAdd);
            } else {
                lines.push(newLine);
                newLine = [toAdd];
...
function lineHeight(){
    return data.env.getFontSize()*1.3;
}

class ChangelogScene extends ScrollableScene<any> {
    constructor() {
        super({min: 0});
    }
    ...
    render(env:AnyEnhancedEnv) {
        ...
        for(let line=0;line<lines.length;line++){
            env.spFillText(lines[line]!, 0, 4+line*lineHeight(), {align:"left",baseline:"top"});
        }
