    ...
    static apply(elements:Element[], env:BoardEnv, historicalNodes:NodeGroup = {}){
        ...
        for (let passData of passes) {
            ...
            if (data) {
                ...
                for (const id of Object.keys(data.nuked)) {
                    nodes[id]!.detach();

                    delete nodes[id];
                    delete historicalNodes[id];
                }
            }
        }
        return nodes;
        ...
        return {
            func:(nodes, env, historicalNodes) => {
                ...
                if(self.editing){
                    ...
                    let offs = new Vector(origCenterX,pos.y);
                    ...
                    self.editDialog.getAbsolutePos = ()=>
                        offs.add(env.board.getAbsolutePos());
...
class ApplyData {
    public readonly added:{[key:string]:Node<any, any>} = {};
    public readonly removed:{[key:string]:Node<any, any>} = {};
    public readonly nuked:{[key:string]:Node<any, any>} = {};
    constructor({added = new Set(), removed = new Set(), nuked = new Set()}:
                {added?:Set<Node<any, any>>, removed?:Set<Node<any, any>>, nuked?:Set<Node<any, any>>}={}) {
        this.added = {};
        for (const node of added) this.added[node.id] = node;
        this.removed = {};
        for (const node of removed) this.removed[node.id] = node;
        this.nuked = {};
        for (const node of nuked) this.nuked[node.id] = node;
    }
    ...
    nuke(node:Node<any, any>){
        this.nuked[node.id] = node;
        return this;
    }
}
